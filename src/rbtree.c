...
void rbtree_traversal(struct rbtree_node* root, void(*visit)(struct rbtree_node*))
{
	if (root == NULL) return;
	struct rbtree_node** stack = (struct rbtree_node**)malloc(RBT_MAX_DEPTH*sizeof(struct rbtree_node*));
	int top = 0;
	...
	while (node || top > 0) {
		if (node != NULL) {
			...
			node = stack[--top];
			struct rbtree_node* next = node->right;
			visit(node);
			node = next;
		}
	}
...
void free_node(struct rbtree_node* node) {
	if (node == NULL) return;
...
void rbtree_destroy(struct rbtree* tree) {
	if (tree == NULL) return;
	rbtree_traversal(tree->root,&free_node);
